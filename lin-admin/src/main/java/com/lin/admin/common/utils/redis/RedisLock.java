package com.lin.admin.common.utils.redis;
...
public class RedisLock {
     ...
     * 加锁
     *
     * @param key   接口提交的业务唯一标识（网关建全header里获取）和调用方身份标识（businessKey）组合成一个唯一标识
     * @param value 当前时间+超时时间 也就是时间戳，单位毫秒
     * @return
     */
    public boolean lock(String key, String value) {
        ...
        if (StringUtils.isNotBlank(currentValue) && Long.parseLong(currentValue) < System.currentTimeMillis()) {
            // currentValue不为空且小于当前时间, 获取上一个锁的时间value
